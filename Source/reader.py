...
def get_dependencies_path(txt_file):
    ...
    dependencies += re.findall(expression_wav, ext_block)
...
def get_dependencies_path_gd(txt_file):
    ...
    dependencies += re.findall(expression_wav, txt_file)
...
def collect_dependencies(scene_path):
    ...
    if not exists(scene_path) or "png" in scene_path or "shader" in scene_path or "mp4" in scene_path or "wav" in scene_path:
        return     
    sub_dependencies = []
    all_paths = []
    #first read the scene path to extract required dependencies
    f = open(scene_path, "r", encoding=None)
    txt = f.read()
    f.close()
    if ".gd" in scene_path:
